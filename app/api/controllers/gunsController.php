...
class GunsController // should have fused this with favourite controller, :(
{
    private $gunsService;
    ...
    public function displayGuns()
    {
        try {
            if ($_SERVER['REQUEST_METHOD'] == 'GET') {
                $guns = $this->gunsService->getGunsToDisplayInGunsPage();
    ...
    public function addGunToFavourites()
    {
        try {
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                ...
                if (isset($input['userId'], $input['gunId'])) {
                    ...
                    $this->gunsService->addGunToFavourites($userId, $gunId);
                    echo json_encode(['success' => true, 'message' => 'Successfully added to favourites']);
                } else {
                    http_response_code(400);
